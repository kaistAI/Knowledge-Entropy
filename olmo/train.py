...
class Trainer:
    ...
    def __post_init__(self):
        ...
        self.initial_temperature_attention, self.initial_temperature_MLP= None, None
        if self.cfg.model.temperature_attention is not None:
            log.warning(f"Temperature applied to Attention with initial value {self.cfg.model.temperature_attention}!")
            self.initial_temperature_attention = self.cfg.model.temperature_attention

        if self.cfg.model.temperature_MLP is not None:
            log.warning(f"Temperature applied to MLP with initial value from {self.cfg.model.temperature_MLP}!")
            self.initial_temperature_MLP = torch.load(self.cfg.model.temperature_MLP).to(self.device)

        # Define a dictionary to accumulate the gradients
        self.gradient_sums = {}
    @property
    ...
    def temperature_schedule(self, step, max_steps, initial_temp=1.0, final_temp=1.0):
        if self.cfg.model.temperature_decay:
    ...
    def tensor_temperature_schedule(self, new_tensor, step, max_steps):
        if self.cfg.model.temperature_decay:            
            final_temp = 1.0
            new_tensor = new_tensor.clone().to(self.device)
    ...
    def model_forward(
        ...
        temperature_attention, temperature_mlp= None, None
        if self.initial_temperature_attention is not None:
            temperature_attention = self.temperature_schedule(self.global_step, self.max_steps, initial_temp=self.initial_temperature_attention)
        if self.initial_temperature_MLP is not None:
            temperature_mlp = self.tensor_temperature_schedule(self.initial_temperature_MLP, self.global_step, self.max_steps)
        # shape: (batch_size, seq_len, vocab_size)
        logits = self.fsdp_model(
            input_ids=batch["input_ids"],
            attention_mask=batch.get("attention_mask"),
            attention_bias=batch.get("attention_bias"),
            temperature_attention=temperature_attention,
    ...
    def fit(self):
        ...
        log.info(f"Max Epoch - {self.max_epochs}") 
